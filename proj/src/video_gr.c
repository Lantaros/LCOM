...
void *vg_init(unsigned short mode, unsigned int* buff_size)
{
	struct reg86u reg;
...
//------------------------------------------------------------------------------
	...
	int r;
	struct mem_range mr;
	unsigned int vram_base = (phys_bytes) mode_info.PhysBasePtr; /* VRAM's physical addresss */	//0xE0000000
	unsigned int vram_size = h_res * v_res * (bits_per_pixel / 8); /* VRAM's size, but you can use8
