...
void convert_packet_struct(unsigned char packet[3], mouse_t* mouse)
{
		...
		mouse->y_overflow = packet[0] & BIT(7);
		mouse->x_overflow = packet[0] & BIT(6);

		mouse->y_sign = packet[0] & BIT(5);
		mouse->x_sign = packet[0] & BIT(4);

		mouse->middle_button = packet[0] & BIT(2);
		mouse->right_button = packet[0] & BIT(1);
		mouse->left_button = packet[0] & BIT(0);

	if (mouse->x_sign) // X
		mouse->delta_x = 0xFF00 | packet[1];
	else
		mouse->delta_x = packet[1];

	if (mouse->y_sign) // Y
		mouse->delta_y = 0xFF00 | packet[2];
	else
...
{
	printf("   LB=%d", mouse->left_button);
	printf("   MB=%d", mouse->middle_button);
	printf("   RB=%d\n", mouse->right_button);

	printf("DeltaX: %d  ", mouse->delta_x);
	printf("DeltaY: %d\n\n", mouse->delta_y);
}
