...
//these functions allow access to I/O ports

//int sys_inb(port_t port, unsigned long *byte); -> read a value into "byte" from port
//int sys_outb(port_t port, unsigned long byte); -> write a value "byte" into port

//an interrupt handler does not take any arguments and does not return any value
...
//  hook_id should be initialized with a value that will be used in notifications
//  return value must be used in other calls to specify the interrupt notification to operate on

//int sys_irqmpolicy(int *hook_id);
//  unsubscribes a previous subscription of the interrupt notification associated with the hook_id
...
int hook_id = 0; //Global variable, accessible by all functions
