...
{
	unsigned long stat_val, trash;
	unsigned int tries = TIMEOUT;

	if (sys_inb(KBD_STAT_REG, &stat_val) != OK)
	{
		printf(
				"Error in clear_in_buf: error in sys_inb(): checking if stat_reg is empty\n");
		return 1;
	}
	while (stat_val & IN_BUF_FULL && tries)
	{
		//tickdelay(micros_to_ticks(DELAY_US));
...
int clear_out_buf(void)
{
	...
	while (stat_val & OUT_BUF_FULL && tries)
	{
		tickdelay(micros_to_ticks(DELAY_US));
...
int write_command_leds(unsigned long leds)
{
	...
	{
		printf(
				"Error in write_command_leds: error in sys_outb: processing ACK response\n");
		return 1;
